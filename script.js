const STOCK_DB=[{"code": "600519", "name": "贵州茅台", "market": "沪市主板"}, {"code": "601318", "name": "中国平安", "market": "沪市主板"}, {"code": "600036", "name": "招商银行", "market": "沪市主板"}, {"code": "600900", "name": "长江电力", "market": "沪市主板"}, {"code": "601899", "name": "紫金矿业", "market": "沪市主板"}, {"code": "603259", "name": "药明康德", "market": "沪市主板"}, {"code": "000001", "name": "平安银行", "market": "深市主板"}, {"code": "000333", "name": "美的集团", "market": "深市主板"}, {"code": "000858", "name": "五粮液", "market": "深市主板"}, {"code": "002594", "name": "比亚迪", "market": "深市主板"}, {"code": "002415", "name": "海康威视", "market": "深市主板"}, {"code": "300750", "name": "宁德时代", "market": "创业板"}, {"code": "300760", "name": "迈瑞医疗", "market": "创业板"}, {"code": "300059", "name": "东方财富", "market": "创业板"}, {"code": "300124", "name": "汇川技术", "market": "创业板"}, {"code": "301199", "name": "迈赫股份", "market": "创业板"}, {"code": "688981", "name": "中芯国际", "market": "科创板"}, {"code": "688041", "name": "海光信息", "market": "科创板"}, {"code": "688111", "name": "金山办公", "market": "科创板"}, {"code": "688256", "name": "寒武纪", "market": "科创板"}, {"code": "920211", "name": "新睿电子", "market": "北交所"}, {"code": "920510", "name": "丰光精密", "market": "北交所"}, {"code": "920193", "name": "吉冈精密", "market": "北交所"}, {"code": "920096", "name": "嘉晨智能", "market": "北交所"}, {"code": "920002", "name": "万达轴承", "market": "北交所"}];
...
function verify(){
 ...
 if(!/^\d{6}$/.test(code)) return showVerify({valid:false,type:"bad",title:"❌ 代码格式错误",msg:"请输入 6 位 A股代码。",stock:null});
 const stock=stockByCode(code);
 if(stock){
   if(name && name!==stock.name) return showVerify({valid:false,type:"bad",title:"❌ 名称与代码不匹配",msg:`${code} 对应「${stock.name}」，不是「${name}」。`,stock});
   if(!name) $("name").value=stock.name;
   return showVerify({valid:true,type:"ok",title:"✅ 数据库验证通过",msg:`代码：${stock.code}\n名称：${stock.name}\n市场：${stock.market}`,stock});
 }
 const market=guessMarket(code);
 if(market!=="疑似无效") return showVerify({valid:false,type:"warn",title:"⚠️ 格式可能有效，但数据库未收录",msg:`${code} 符合「${market}」代码段，但 V1.3 内置数据库暂未收录。`,stock:null});
 return showVerify({valid:false,type:"bad",title:"❌ 疑似无效代码",msg:`${code} 不符合常见 A股代码段。`,stock:null});
}
...
function riskBy(score,big,rank){
...
function analyze(){
 ...
 const risk=riskBy(score,bp.type,mr.type);
 ...
 $("maRank").textContent=mr.type; $("maHint").textContent=mr.msg;
...
五、版本说明
V1.3 已把你的选股体系写进评分逻辑：
先看大位置，再看 MA5 > MA10 > MA20 > MA60，最后看 MA20距离、成交量和风险。
V1.4 再做联网行情接口，自动拉取真实价格和均线。`;
}
function resetAll(){
 document.querySelectorAll("input").forEach(i=>i.value="");
 ["score","bigPos","maRank","risk"].forEach(id=>$(id).textContent="--");
 $("scoreText").textContent="等待分析"; $("bigHint").textContent="历史区间"; $("maHint").textContent="MA5/10/20/60"; $("riskHint").textContent="等待分析";
