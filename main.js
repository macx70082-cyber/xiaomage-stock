...
function run(){
  ...
  document.getElementById('summaryCards').innerHTML = card('综合结论',score.decision,score.decision==='偏多'?'ok':score.decision==='偏空'?'danger':'warn')+card('风险等级',risk.grade,risk.grade==='D'||risk.grade==='E'?'danger':'')+card('多空评分',`${score.bull} : ${score.bear}`)+card('位置评分',`${pos.positionScore}/10`)+card('案例匹配',`${mc.sim}%`);
  document.getElementById('report').classList.remove('empty');
  document.getElementById('report').innerHTML =
    block('【基础验证】',`<p>${d.name}（${d.code}）：${codeQuality(d.code)}</p>`)+
    block('【位置分析】',`<p><b>大位置：</b>${pos.big}。${pos.bigExplain}</p><p><b>小位置：</b>${pos.small}。${pos.smallExplain}</p><p><b>最终位置判断：</b>${pos.finalPosition}。${pos.priorityExplain}</p><p>位置评分：<b>${pos.positionScore}/10</b>；位置风险：<b>${pos.positionRisk}/5</b>。</p><p>当前状态：${pos.maState}；距离MA20：${pos.ma20Gap.toFixed(2)}%，距离MA60：${pos.ma60Gap.toFixed(2)}%。</p><p><b>交易员点评：</b>${pos.traderComment}</p>`)+
